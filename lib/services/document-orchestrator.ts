  ...
  async generateDocument(request: OrchestrationRequest): Promise<OrchestrationResult> {
    ...
    try {
      ...
      for (let i = 0; i < structure.length; i++) {
        ...
        if (
          request.documentType === 'Protocol' &&
          section.section_id === 'protocol_schedule_of_assessments'
        ) {
          try {
            const studyFlow =
              (this as any)._studyFlow ||
              (await this.fetchStudyFlowFromDb(request.projectId, supabase)) ||
              (await generateStudyFlowForProject(request.projectId, supabase))

            if (!studyFlow) {
              throw new Error('Study Flow not available')
            }

            sections[section.section_id] = this.renderSOAFromStudyFlow(studyFlow)
          } catch (error) {
            console.error(
              `❌ Deterministic SoA generation failed for ${section.section_id}. This section will not be generated by LLM to preserve consistency.`,
              error
            )

            sections[section.section_id] = `## Schedule of Assessments

[Generation Error for protocol_schedule_of_assessments]

Error: ${error instanceof Error ? error.message : 'Unknown error'}

This section is generated deterministically from Study Flow. Ensure Study Flow generation succeeds and that the project has a valid visit schedule and procedures.`
          }
  ...
  private async fetchStudyFlowFromDb(projectId: string, supabase: any) {
    try {
  ...
  private renderSOAFromStudyFlow(studyFlow: any): string {
    const visitsRaw = Array.isArray(studyFlow?.visits) ? studyFlow.visits : []
    const proceduresRaw = Array.isArray(studyFlow?.procedures) ? studyFlow.procedures : []

    const visits = [...visitsRaw].sort((a, b) => {
      const ad = typeof a?.day === 'number' ? a.day : 0
      const bd = typeof b?.day === 'number' ? b.day : 0
      return ad - bd
    })

    const visitHeaders = visits
      .map((v: any) => {
        const name = v?.name || v?.label || v?.id || 'Visit'
        const day = typeof v?.day === 'number' ? `Day ${v.day}` : ''
        return day ? `${name}<br/>${day}` : name
      })
      .join(' | ')

    const headerLine = `| Procedure | ${visitHeaders} |`
    const separatorLine = `| --- | ${visits.map(() => '---').join(' | ')} |`

    const categoriesOrder = ['administrative', 'safety', 'efficacy', 'other']
    ...
    const normalizeCategory = (cat: any): string => {
      ...
      if (c.includes('efficacy')) return 'efficacy'
      return 'other'
    }

    const allProcedures = proceduresRaw
    ...
    for (const cat of categoriesOrder) {
      const catProcs = allProcedures.filter((p: any) => p.category === cat)
      if (catProcs.length === 0) continue

      rows.push(`| **${categoryLabels[cat]}** | ${visits.map(() => '').join(' | ')} |`)

      for (const proc of catProcs) {
        const cells = visits
          .map((v: any) => {
            const visitProcIds: string[] = Array.isArray(v?.procedures) ? v.procedures : []
            if (visitProcIds.includes(proc.id)) return 'X'

            const top = studyFlow?.topMatrix || studyFlow?.top_matrix
            if (top && typeof top === 'object') {
              if (
                Array.isArray((top as any).visits) &&
                Array.isArray((top as any).procedures) &&
                Array.isArray((top as any).matrix)
              ) {
                const vIdx = (top as any).visits.findIndex((tv: any) => tv?.id === v?.id)
                const pIdx = (top as any).procedures.findIndex((tp: any) => tp?.id === proc.id)
                const val = (top as any).matrix?.[vIdx]?.[pIdx]
                return val ? 'X' : ''
              }

              if (v?.id && (top as any)[v.id] && typeof (top as any)[v.id] === 'object') {
                return (top as any)[v.id]?.[proc.id] ? 'X' : ''
              }
            }
          ...
          .join(' | ')
        rows.push(`| ${proc.name} | ${cells} |`)
      }
    }

    const totalDuration = typeof studyFlow?.totalDuration === 'number' ? studyFlow.totalDuration : null
    const durationLine = totalDuration
      ? `**Planned total duration:** ${totalDuration} days (${Math.round(totalDuration / 7)} weeks)`
      : ''

    return `## Schedule of Assessments
